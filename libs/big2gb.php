<?php
	...
	class BIG2GB{
		...
		function str_simple(){
			...
			return array($cn,$big5);
		}

		function go($s,$customSimple=false){
			$n=strlen($s);
			if($customSimple === false){
				list($cn,$big5) = self::str_simple();
			}else{
				list($cn,$big5) = $customSimple;
			}
		...
		function reverse($s){
			list($cn,$big5) = self::str_simple();
			return self::go($s,array($big5,$cn));
		}
	}
